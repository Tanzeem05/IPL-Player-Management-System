...
public class HomepageUpdater {
    ...
    synchronized void search() {
        ...
        if (searchOption == 0) {
            ...
            if (searchString.strip().isEmpty()) return;
            updateGUI(localDatabase.searchPlayerByCountryOrClub(searchString));

        }
        else {
            Platform.runLater(() -> homepageController.setMessageLabel("Invalid search option"));
        }
            ...
            public void run() {
                ...
                for (int i = 0; i < players.size(); i++) {
                    ...
                    if (list == 1) {
                        ...
                        boolean isClubPlayer = localDatabase.getClubPlayers().contains(players.get(i));
                        playerBox.inti(players.get(i), isClubPlayer);
                    }
                    else if(list == 2) {
